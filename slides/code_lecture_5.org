...
- Bias: bias decreases as a function of model complexity
...
The idea is this:
- With CR we search for (a) good---searching for optimality is nearly always a mathematical deviation (disorder), based on too much playing with (believing in ) in toy models---hyper-parameter(s).
- Once we found good hyper-parameters we fit the model on all available data.
...
Below is my explanation, so in case you want to derive things yourself, stop reading.
...
\end{align}
Clearly, we want to find $s^{*}$ such that this is minimized.
Let us rewrite this problem into more general form, so that we can see how include weights.
...
Now observe that we replace the $m(L)$ and $m(R)$ by other impurity measures, for instance the Gini measure,
\begin{equation}
\label{eq:2}
G(A) = \frac{1}{2} \left( 1 - p_{-1}(A)^{2} - p_{1}(A)^{2} \right),
\end{equation}
for a set $A$. In that case, we want to find an $s$, that induces sets $L$ and $R$ that minimize
\begin{align}
\label{eq:1}
...
\end{equation} With this, the /weighted score/ of the separator $s$ that splits the set of data points into subsets $L$ and $R$ becomes
\begin{equation}
\label{eq:1}
...
In the bagging code, from which I learned a lot BTW, the author mixes the /construction/ of a bag with the /testing/ of bags. In my opinion that is a bad idea. These two parts of software should be split.

* Exercises
** Exercise 1,on code
- Read  [[https://www.geeksforgeeks.org/why-import-star-in-python-is-a-bad-idea/][here]] to understand why using ~import *~ is a very bad idea. Let me be honest: DSML use this style quite a bit. What annoys me slightly about the code in the DSML is that they have very different standards about how math should look like (impeccable), and how code should look like (a bit ugly, bad formatting here and there). This is just as awkward in the other way around, correct code and wrong math.
...
- You might also like to  check this [[https://www.betterdatascience.com/mml-random-forest/][site]]. There is a point of confusion: does the author call /random forest/ what we call /bagging/? I think so, but I haven't  studied the code  in detail, so I might be wrong.
